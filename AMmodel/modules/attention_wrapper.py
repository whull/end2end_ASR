...
def _location_based_score(W_query, keys, attention_weights, conv_k, conv_r):
  """
  ...
  W_query = array_ops.expand_dims(W_query, 1)

  # [batch_size, max_time] -> [batch_size, max_time, 1]
  attention_weights = tf.expand_dims(attention_weights, axis=2)
  # location features [batch_size, max_time, filters]
  f = tf.layers.conv1d(attention_weights, filters=conv_r,
                        kernel_size=conv_k, padding='same',
  ...
  return tf.reduce_sum(v_a * (tf.tanh(keys + W_query + W_fil)), axis=2)
  ...
  577–585.
    ...
    def __call__(self, query, previous_alignments):
        ...
        with variable_scope.variable_scope(None, "location_based_attention", [query]):
            ...
            energy = _location_based_score(processed_query, self._keys, previous_alignments, self.conv_k, self.conv_r)
